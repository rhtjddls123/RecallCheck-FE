...
const RecallResultCard = ({ product, foundInOtherCategory }: RecallResultCardProps) => {
  ...
  return (
    <div className="select-none mt-2 rounded-2xl border border-red-100 bg-white shadow-sm overflow-hidden w-72">
      {/* 상단 배지 바 */}
      ...
      <div className="px-4 py-3 space-y-2">
        <div className="flex justify-end">
          <Link className="text-11_M underline" href={`/recall/${product.recallSn}`}>
            상세보기
          </Link>
        </div>
        ...
        <div className="flex justify-between text-xs">
          <span className="text-gray-400">리콜일</span>
          <span className="text-gray-600">{`${recallDate.from} ~ ${recallDate.to}`}</span>
        </div>
        <div className="pt-2 border-t border-gray-100">
          <p className="text-xs text-gray-400 mb-1">리콜 사유</p>
          <p className="text-xs text-red-500 font-medium leading-relaxed truncate h-[19.5px]">
            {product.shrtcomCn || product.injryCauseResult}
          </p>
        </div>
        <div className="pt-2 border-t border-gray-100">
          <p className="text-xs text-gray-400 mb-1">조치 사항</p>
          <p className="text-xs text-gray-600 leading-relaxed truncate h-[19.5px]">
